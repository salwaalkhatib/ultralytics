...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', type=str, default='runs/detect/v8s-logit_calib', help='path to dataset root')
    parser.add_argument('--experiment', type=str, default='contr_momen_', help='Experiment name head')
    ...
    model = YOLO(CKPT)
    model.train(data="isaid.yaml", epochs=EPOCHS, batch=BATCH_SIZE, save_period=9, workers=WORKERS, name=EXPERIMENT, 
                project=PROJECT, contr_loss=CONTRASTIVE_LOSS, mosaic=MOSAIC_PROB, close_mosaic=CLOSE_MOSAIC,
                contr_pnorm=CONTRASTIVE_PNORM, contr_warmup=CONTR_WARMUP, contr_ema_iters=CONTR_EMA_ITERS, queue_size=QUEUE_SIZE, contr_calib=CONTR_CALIB, val=False)
